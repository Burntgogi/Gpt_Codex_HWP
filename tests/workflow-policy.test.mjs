...
test("CI runs bounded platform diagnostics only after the matching release gate fails", async () => {
  ...
  const largeEvidenceDiagnosticCommand = "node packages/gpt-codex-hwp/benchmarks/document-engine-benchmark.mjs --validate-large .superpowers/benchmarks/release-large.json";
  ...
  const largeEvidenceDiagnostic = macos.indexOf(largeEvidenceDiagnosticCommand);
  ...
  assert.equal(
    largeEvidence >= 0 && largeEvidence < macosControl && macosControl < macosReleaseGate
      ...
      && documentBenchmarkDiagnostic < largeEvidenceDiagnostic
      && largeEvidenceDiagnostic < macosForensicDiagnostic
