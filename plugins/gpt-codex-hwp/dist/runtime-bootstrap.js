...
export async function resolveManagedRuntime(importMetaUrl, options = {}) {
    let entryPath;
    try {
...
export async function readVerifiedManagedRuntimeFile(identity, record) {
    ...
    let bytes;
    try {
        ({ bytes } = await readOwnedBytes(identity.pluginRoot, record.path, MAX_MANIFEST_FILE_BYTES));
    }
    catch (error) {
...
export async function resolveInstalledRuntime(importMetaUrl, mainEntry, options = {}) {
    if (!MAIN_ENTRIES.includes(mainEntry))
        throw failure("RUNTIME_PATH_INVALID");
    ...
    try {
        ...
        if (isMissing(error))
            throw failure("RUNTIME_NOT_INSTALLED");
        if (error instanceof RuntimeBootstrapError)
            throw error;
        throw failure("RUNTIME_PATH_INVALID");
    }
    ...
    try {
        ...
        for (const dependency of identity.directDependencies) {
            if (dependency === "kordoc")
                await requireKordocDependency(root);
            else
                await requireOwnedRegularFile(root, packageJsonPath(dependency));
        }
    }
    catch {
...
async function requireKordocDependency(root) {
...
function parseManifest(bytes, pluginVersion, lockHash) {
    let parsed;
    try {
...
async function readManagedJson(root, path, maximumBytes) {
...
async function readManagedBytes(root, path, maximumBytes) {
    try {
        return await readOwnedBytes(root, path, maximumBytes);
    }
    catch (error) {
        if (error instanceof RuntimeBootstrapError)
            throw error;
...
async function readOwnedJson(root, path, maximumBytes) {
    const { bytes } = await readOwnedBytes(root, path, maximumBytes);
    return JSON.parse(new TextDecoder().decode(bytes));
}
async function readOwnedBytes(root, path, maximumBytes) {
...
async function requireOwnedRegularFile(root, path) {
...
async function requireExactDirectory(path) {
    ...
    if (!status.isDirectory() || status.isSymbolicLink())
        throw failure("RUNTIME_PATH_INVALID");
    if (!samePath(resolve(path), await realpath(path)))
        throw failure("RUNTIME_PATH_INVALID");
}
...
function durablePath(path) {
    if (!safeRelative(path) || path === "runtime-manifest.json" || path.startsWith("node_modules/"))
        return false;
    return new Set([
...
function major(version) {
    ...
    if (!Number.isSafeInteger(value) || value < 22)
        throw failure("RUNTIME_PLATFORM_MISMATCH");
    return value;
}
function hash(value) {
