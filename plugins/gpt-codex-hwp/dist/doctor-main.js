...
async function createDefaultDependencies() {
    ...
    return {
        ...
        verifyKordocRuntime: async () => {
            ...
            const installedFiles = await verifier.kordocFileRecords(join(runtimeRoot, "node_modules", "kordoc"));
...
export async function probeRegisteredToolSchemasInProcess() {
...
async function exactToolSchemas(dependencies, tools) {
...
function canonicalSchemaValue(value, state, depth, mode) {
    state.nodes += 1;
    if (state.nodes > 2_048 || depth > 16)
        throw new Error("tool schema exceeds structure bound");
    if (value === null || typeof value === "string" || typeof value === "boolean")
        return value;
    if (typeof value === "number" && Number.isFinite(value))
        return value;
    if (Array.isArray(value)) {
        return value.map((entry) => canonicalSchemaValue(entry, state, depth + 1, mode === "schema-list" ? "schema" : mode));
    }
    if (typeof value !== "object")
        throw new Error("tool schema contains non-JSON value");
    const source = value;
    const result = {};
...
async function executeWindowsBoundedCommand(specification, runtimeRoot, dependencies) {
    ...
    if (startupResult.kind === "deadline") {
        ...
        if (!unsafeDoctorStartupRetentions.has(child))
...
function dispatchDoctorRunnerInput(child, input, frame) {
    return new Promise((resolveDispatch) => {
        ...
        try {
            input.end(frame, (error) => finish(error));
        }
        catch (error) {
            finish(error);
        }
...
async function terminateGatedRunner(child, supervisor, closeReceipt) {
    if (supervisor.ok)
        return terminateSupervised(supervisor.value);
...
function terminateSupervised(supervisor) {
