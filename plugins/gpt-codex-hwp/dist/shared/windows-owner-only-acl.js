...
export async function applyWindowsOwnerOnlyAcl(path, kind, options = {}) {
    ...
    try {
        ...
        result = await run(command, ["-NoProfile", "-NonInteractive", "-Command", WINDOWS_OWNER_ONLY_ACL_SCRIPT], {
            encoding: "utf8",
    ...
    if (result.stderr !== "" || !ALLOWED_RESULTS.has(result.stdout)) {
        return "invalid-output";
    }
    return result.stdout;
}
export function createWindowsAclHelperEnvironment(path, kind, source = process.env) {
    const result = {
...
export function resolveWindowsAclPowerShell(platform = process.platform, systemRoot) {
    if (platform !== "win32")
...
async function runWindowsAclPowerShell(command, args, options) {
