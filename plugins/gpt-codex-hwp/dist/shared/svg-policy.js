...
export class IncrementalSvgPolicyValidator {
    ...
    #pushTagCharacter(character) {
        ...
        if (character === "<")
            throw new Error("SVG tag structure is invalid.");
...
export function assertSafeSvgString(svg) {
    if (typeof svg !== "string")
        throw new Error("SVG output is not text.");
    const validator = new IncrementalSvgPolicyValidator();
    validator.push(svg);
    validator.finish();
}
function pushBase64Character(state, character) {
    if (character === "=") {
        state.padding += 1;
        if (state.padding > 2)
            throw new Error("SVG data image has invalid base64 padding.");
        return;
    }
...
function assertCompleteBase64(state) {
...
function base64Sextet(character) {
...
function inspectSvgTag(tag) {
    ...
    for (const match of lower.matchAll(/\b(?:href|xlink:href|src)\s*=\s*(["'])(.*?)\1/gu)) {
...
function readSvgAttributes(tag) {
    const attributes = [];
    let offset = 1;
    while (offset < tag.length && /\s/u.test(tag[offset]))
        offset += 1;
    if (tag[offset] === "/")
        offset += 1;
    while (offset < tag.length && !/[\s>]/u.test(tag[offset]))
    ...
    while (offset < tag.length) {
        while (offset < tag.length && /\s/u.test(tag[offset]))
            offset += 1;
        if (tag[offset] === "/" || tag[offset] === ">" || offset >= tag.length)
            break;
        ...
        if (quote === "\"" || quote === "'") {
            ...
            while (offset < tag.length && tag[offset] !== quote)
                offset += 1;
...
function hasUnsafeNamespaceDeclaration(attributes) {
    return attributes.some(({ name, value }) => (name === "xmlns" && value !== SVG_NAMESPACE) ||
        (name.startsWith("xmlns:") &&
            (name !== "xmlns:xlink" || value !== XLINK_NAMESPACE)));
}
function decodeXmlEntities(value) {
    const decoded = value.replace(/&(?:#x([0-9a-f]{1,6})|#([0-9]{1,7})|(amp|lt|gt|quot|apos));/giu, (entity, hex, decimal, named) => {
        if (named !== undefined) {
            return { amp: "&", lt: "<", gt: ">", quot: "\"", apos: "'" }[named.toLocaleLowerCase("en-US")];
        }
