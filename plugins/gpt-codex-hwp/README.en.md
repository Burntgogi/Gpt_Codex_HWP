...
<p align="center">
  ...
  <a href="#stable-v025-installation-from-github">Quick install</a> ·
...
`v0.2.5` retains v0.2.2's read-only HWP policy, HWPX writing, explicit durable runtime installation, and all nine one-shot tool contracts. The release gate now runs the exact complete inventory of 26 root and 41 source test files through bounded per-file isolation after cumulative execution proved unstable on hosted Windows. Inventory drift fails closed, failures expose only fixed privacy-safe receipts, and Windows descendant process trees are terminated and reclaimed within a bound. No tests or coverage were removed, and user runtime, tool, and document behavior are unchanged from the `v0.2.4` candidate. Development and real-document validation were performed on Windows x64. macOS Apple Silicon CI is judged only by a successful current-HEAD receipt; Codex Desktop and Hancom Office Hangul on a physical Mac remain unverified.

## Previous releases

`v0.2.3` and `v0.2.4` are immutable unpublished candidate tags; neither tag has a GitHub Release or distribution assets. `v0.2.2` is the previous public stable release and introduced read-only HWP, HWPX writing, default one-shot execution, and the 100 MiB CI-verified envelope. See the [changelog](CHANGELOG.md) and [English release notes](RELEASE_NOTES.en.md) for retained history.
