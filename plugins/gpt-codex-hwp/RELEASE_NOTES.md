...
| 공개 MCP 도구 | 9개 | 9개 유지 | 기능이나 에이전트 명령을 줄이지 않음 |
...
| 사용자 설치 크기 | Windows x64 운영 의존성 약 44.38 MiB | 약 44.13 MiB | 약 0.6% 감소로 체감 가능한 차이는 아님 |
| 장기 세션 메모리 | 비교 기준 없음 | 비교 기준 없음 | 메모리 사용량 감소를 보장하지 않음 |

## 사용자 자원 사용 안내

v0.2.2는 사용자 컴퓨터의 정상 상태 Node 메모리나 설치 공간을 크게 줄인
릴리즈가 아닙니다. Windows x64에서 동일한 운영 설치 명령으로 측정한
`node_modules`는 약 44.38 MiB에서 44.13 MiB로 줄었지만 차이는 약 0.6%에
불과합니다. MCP 도구 9개도 그대로이므로 이 수치를 사용자 자원 최적화 성과로
과장하지 않습니다.

대신 작업 종료 검증을 강화했습니다. 최신 설치 런타임 스모크는 도구 실행 후
`remainingDescendants=0`을 확인했으며, 등록된 자식 프로세스가 남으면 성공으로
처리하지 않습니다. 이는 반복 작업에서 프로세스와 메모리가 누적될 위험을 낮추는
안전장치지만, 장시간 실제 Codex 세션의 RSS 메모리가 일정 비율 감소한다는 뜻은
아닙니다. 해당 비교 측정치는 아직 없습니다.

이번 버전에서 크게 줄어든 것은 유지관리용 CI 사용량입니다. 일반 PR의 중복
대용량 검증을 제거했지만 사용자 기능을 줄인 변경은 아닙니다. 실행 시간과 runner
사용량을 포함한 상세 수치는
[개발 문서](https://github.com/Burntgogi/Gpt_Codex_HWP/blob/main/docs/DEVELOPMENT.md#ci-resource-measurement)에
분리해 기록했습니다.
