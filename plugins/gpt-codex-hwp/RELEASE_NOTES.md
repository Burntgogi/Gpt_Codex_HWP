...
v0.2.4는 v0.2.2의 HWP 읽기 전용·HWPX 쓰기 원칙과 내부 one-shot 도구 계약 9개를 유지하면서, Codex 관리 캐시가 재생성되어도 의존성이 유지되는 명시적 런타임 설치를 추가합니다. 불변 `v0.2.3` 태그는 hosted Windows 릴리스 검증의 이식성 문제를 확인한 뒤 미게시 후보로 보존했으며 GitHub Release와 배포 자산은 만들지 않았습니다. v0.2.4는 이 검증 문제만 바로잡으며 v0.2.3 후보 대비 사용자 런타임이나 도구 동작을 바꾸지 않습니다.
...
- 환경과 경로 길이에 따라 달라지는 stderr 바이트 수를 특정 값 하나가 아니라 `1`부터 `65,537`까지의 경계로 검증합니다. 고정 stage·boundary, 원시 출력 비노출과 자식 프로세스 종료 검사는 그대로 유지합니다.
...
- 공개 안정판 설치 안내를 v0.2.4와 기본 one-shot 구조에 맞췄습니다. 기본 설치는 `/mcp`에 영구 `gpt-codex-hwp` 서버를 등록하지 않습니다.
- v0.2.2 공개 릴리스와 미게시 v0.2.3 후보 태그는 변경하지 않았으며 v0.2.4는 새 빌드 ID를 사용합니다.
...
v0.2.4를 설치할 때는 설치 결과의 `installedPath`를 경로·플러그인 identity 규칙으로 먼저 검증한 뒤 그 경로에서 실행합니다.
