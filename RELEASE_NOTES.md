...
- PR은 10 MiB 스모크와 안정적인 플랫폼 검증을 담당하고, 병합 후 수동·주기 Compatibility 워크플로는 각 플랫폼에서 실제 공개 one-shot 경로의 100 MiB 감지만 담당합니다.
...
- Compatibility에서 필수 CI와 겹치던 전체 테스트·플랫폼 영수증을 제거했습니다. 동일 커밋에서 필수 CI와 100 MiB 스모크는 통과했지만 중복 부하 테스트만 hosted runner 변동으로 실패했기 때문입니다. 전체 릴리즈 검증은 불변 릴리즈 단계가 소유합니다.
...
릴리즈 전 최종 후보는 Windows x64·macOS arm64·Linux lifecycle·Security policy, 병합된 정확한 커밋의 3개 플랫폼 100 MiB production one-shot 스모크, 불변 `v0.2.2` 태그의 릴리즈 후보·배포물·attestation 검증을 모두 통과해야 합니다.
