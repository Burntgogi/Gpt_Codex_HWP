...
export interface HwpCreateSvgAssetInput {
  prompt_or_spec: string;
  output_svg_path: string;
  output_png_path?: string;
}

export interface HwpInsertImageInput {
  file_path: string;
  image_path: string;
  output_path: string;
  anchor_text: string;
...
export interface SvgAssetDependencies {
  validateSvg: (svg: string) => Promise<void>;
  renderSvgToPng: (svg: string) => Promise<Uint8Array>;
}

export interface ImageAssetDependencies {
  parseDocument: (input: string | ArrayBuffer | Buffer, options?: ParseOptions) => Promise<ParseResult>;
  placeSeal: typeof placeSealHwpx;
  validateDocument: (input: ArrayBuffer | Uint8Array) => Promise<ValidateResult>;
  renderDocument: (input: ArrayBuffer | Uint8Array, options?: RenderSvgOptions) => Promise<RenderSvgResult>;
...
const defaultImageDependencies: ImageAssetDependencies = {
  parseDocument: parse,
...
export async function handleHwpInsertImage(
  ...
  dependencyOverrides: Partial<ImageAssetDependencies> = {},
): Promise<CallToolResult> {
  ...
  try {
    ...
    if (input.anchor_text.length > MAX_ANCHOR_CHARACTERS) {
      throw new AssetError(
        "INPUT_TOO_LARGE",
        `anchor_text exceeds the ${MAX_ANCHOR_CHARACTERS}-character safety limit.`,
      );
    }
    ...
    if (!sourceValidation.ok) {
      throw new AssetError(
        "SOURCE_HWPX_INVALID",
        "Source HWPX failed structural validation and was not edited.",
        validationDetails(sourceValidation),
      );
    }
    ...
    const anchor = await selectAnchor(
      zip,
      input.anchor_text,
    ...
    let imageEntry: string;
    let changedSectionEntry: string | undefined;
    ...
    if (mode === "seal-anchor") {
      ...
      if (Array.isArray(metadata.warnings)) {
        warnings.push(...metadata.warnings.filter((value): value is string => typeof value === "string"));
      }
    ...
    if (afterImages !== beforeImages + 1) {
      throw new AssetError(
        ...
        `Expected one inserted image, but image count changed from ${beforeImages} to ${afterImages}.`,
      );
    }
    ...
    if (
      sha256(await readFileBounded(filePath, "source document")) !== sourceHash
    ) {
      throw new AssetError("SOURCE_CHANGED", "The source document changed during image insertion.");
    }
    await writeFilesExclusively(
      [{ path: outputPath, data: candidate }],
...
export function registerHwpCreateSvgAsset(server: McpServer): void {
  server.registerTool(
    HWP_CREATE_SVG_ASSET_TOOL_NAME,
    {
      ...
      description:
        "Create a standalone SVG from sanitized inline SVG or a documented JSON shape specification, with an optional PNG rendering.",
      inputSchema: {
        prompt_or_spec: z
          .string()
          .min(1)
          .max(MAX_SVG_BYTES)
          .describe(
            "Sanitized inline <svg>, or JSON {width,height,viewBox?,background?,elements:[rect|circle|ellipse|line|text|path|polyline]}; shape coordinates use camelCase such as strokeWidth, fontSize, textAnchor.",
          ),
...
export function registerHwpInsertImage(server: McpServer): void {
  server.registerTool(
    HWP_INSERT_IMAGE_TOOL_NAME,
    {
      ...
      description:
        "Normalize a local image to PNG and insert it into a new, structurally validated HWPX after an anchor paragraph or as a seal overlay.",
      inputSchema: {
...
async function normalizeImage(input: Uint8Array): Promise<NormalizedImage> {
...
async function assertEditableHwpx(buffer: ArrayBuffer): Promise<void> {
  try {
    ...
    throw new AssetError(
      "UNSUPPORTED_IMAGE_DOCUMENT_FORMAT",
      `Image insertion supports only a valid HWPX package: ${errorMessage(error)}`,
    );
  }
}
...
async function selectAnchor(
    ...
    for (const paragraph of paragraphs) {
      ...
      while (from <= paragraph.text.length) {
        ...
        if (requestedOccurrence === undefined && matchCount > 1) {
          throw new AssetError(
            "AMBIGUOUS_ANCHOR",
            "Anchor text occurs more than once; set anchor_occurrence to a 0-based occurrence.",
            { anchor_count_at_least: 2 },
          );
        }
  ...
  if (occurrence >= matchCount) {
    throw new AssetError(
      "ANCHOR_NOT_FOUND",
      `anchor_occurrence ${occurrence} is outside 0..${matchCount - 1}.`,
      { anchor_count: matchCount },
    );
  }
  return { occurrence };
}

function eligibleParagraphs(body: ScanParagraph[], tables: ScanTable[]): ScanParagraph[] {
  const paragraphs = [...body];
  const walk = (nestedTables: ScanTable[]): void => {
...
function buildSafeSvg(input: string): string {
...
function sanitizeInlineSvg(svg: string): string {
  ...
  if (namespace !== undefined) return svg;
  return svg.replace(/<svg\b/iu, '<svg xmlns="http://www.w3.org/2000/svg"');
}

function assertSafeInlineSvgVocabulary(svg: string): void {
  if (
    /<\/?\s*[A-Za-z_][\w.-]*:/u.test(svg) ||
...
function inspectDecodedSvg(svg: string, allowedElements: ReadonlySet<string>): void {
  let document;
  try {
...
function inspectSvgElement(element: XmlElement, allowedElements: ReadonlySet<string>): void {
  ...
  const elementAttributes = SVG_ELEMENT_ATTRIBUTES[name] ?? new Set<string>();
  for (let index = 0; index < element.attributes.length; index += 1) {
    const attribute = element.attributes.item(index);
    if (attribute === null) continue;
    const isDefaultNamespace = attribute.name === "xmlns";
    if (
      (!isDefaultNamespace && attribute.prefix !== null) ||
  ...
  for (let child = element.firstChild; child !== null; child = child.nextSibling) {
    if (child.nodeType === 1) {
      inspectSvgElement(child as XmlElement, allowedElements);
...
function renderStructuredSvg(value: unknown): string {
  if (!isRecord(value)) {
...
function renderSvgElement(value: unknown): string {
...
function commonPaintAttributes(value: Record<string, unknown>): string {
  const attributes: string[] = [];
  for (const [key, attr] of [["fill", "fill"], ["stroke", "stroke"]] as const) {
    if (value[key] !== undefined) attributes.push(`${attr}="${escapeXml(safePaint(value[key], key))}"`);
  }
  if (value.strokeWidth !== undefined) attributes.push(`stroke-width="${positive(value.strokeWidth, "strokeWidth")}"`);
  if (value.opacity !== undefined) attributes.push(`opacity="${formatNumber(safeNumber(value.opacity, "opacity", 0, 1))}"`);
  return attributes.length === 0 ? "" : ` ${attributes.join(" ")}`;
}

function safePaint(value: unknown, label: string): string {
...
function numericSvgDimension(attributes: string, name: string): number {
  const match = attributes.match(new RegExp(`\\b${name}\\s*=\\s*["']([0-9]+(?:\\.[0-9]+)?)(?:px)?["']`, "iu"));
  if (match === null) throw new AssetError("UNSAFE_SVG", `Inline SVG requires a numeric ${name}.`);
...
async function preflightOutputPath(outputPath: string, sourcePaths: string[]): Promise<void> {
...
async function runVerifier(
  editedPath: string,
  originalPath?: string,
  allowlist: { changed: string[]; added: string[] } = {
    changed: [],
    added: [],
  },
): Promise<void> {
...
function helperScriptPath(name: string): string {
...
export async function runPython(
  ...
  args: string[],
  platform: NodeJS.Platform = process.platform,
  execute: PythonExecutor = execFilePromise,
): Promise<{ stdout: string; stderr: string }> {
  for (const candidate of pythonCommandCandidates(platform)) {
    try {
      return await execute(
        candidate.command,
        [...candidate.argsPrefix, script, ...args],
        script,
      );
    } catch (error: unknown) {
  ...
  throw new AssetError(
    "PYTHON_NOT_FOUND",
    "Python 3.10 or newer was not found on PATH.",
  );
}

function execFilePromise(command: string, args: string[], script: string): Promise<{ stdout: string; stderr: string }> {
...
function n(value: unknown, label: string): string {
...
function enumString(value: unknown, allowed: string[], label: string): string {
...
function countImages(blocks: readonly IRBlock[]): number {
  let count = 0;
  for (const block of blocks) {
    if (block.type === "image") count += 1;
    if (block.children !== undefined) count += countImages(block.children);
    if (block.table !== undefined) {
      for (const row of block.table.cells) {
        for (const cell of row) {
          if (cell.blocks !== undefined) count += countImages(cell.blocks);
        }
      }
    }
  }
...
function safeResolvedPath(value: unknown): string | undefined {
  try {
...
async function fileSystemIdentity(path: string): Promise<FileSystemIdentity> {
  const stats = await lstat(path, { bigint: true });
  if (!stats.isDirectory() || stats.isSymbolicLink()) {
    throw new UnsafeOutputPathError(
      `Temporary workspace is not an owned directory: ${path}`,
    );
  }
