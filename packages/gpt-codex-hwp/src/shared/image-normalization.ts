...
export type ImageNormalizationErrorCode = "INVALID_IMAGE" | "UNSAFE_SVG";

export class ImageNormalizationError extends Error {
  constructor(readonly code: ImageNormalizationErrorCode, message: string) {
    super(message);
...
export async function normalizeImageBytes(
  ...
  if (bytes.byteLength === 0 || bytes.byteLength > MAX_NORMALIZED_IMAGE_BYTES) {
    throw new ImageNormalizationError(
      "INVALID_IMAGE",
      `Image input must be between 1 and ${MAX_NORMALIZED_IMAGE_BYTES} bytes.`,
    );
  }
  ...
  if (
    ...
    throw new ImageNormalizationError(
      "UNSAFE_SVG",
      "UTF-16 XML/SVG image input is not supported safely.",
    );
  }
  ...
  try {
    ...
    throw new ImageNormalizationError(
      "INVALID_IMAGE",
      `Image could not be decoded safely: ${errorMessage(error)}`,
    );
  }
}

function sanitizeImageSvg(svg: string): string {
...
function inspectSvg(svg: string): void {
  ...
  try {
    ...
    throw new ImageNormalizationError(
      "UNSAFE_SVG",
      `Inline SVG is not well-formed XML: ${errorMessage(error)}`,
    );
  }
...
function inspectElement(element: XmlElement): void {
  ...
  for (let child = element.firstChild; child !== null; child = child.nextSibling) {
    if (child.nodeType === 1) inspectElement(child as XmlElement);
