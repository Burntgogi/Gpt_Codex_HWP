...
export type BoundedZipLoader = (bytes: Uint8Array) => Promise<JSZip>;

export function assertClassicZipEntryBudget(
  bytes: Uint8Array,
  ...
  const observedEntries = countCentralDirectoryEntries(
    view,
    centralOffset,
    centralSize,
    maximumEntries,
  ...
  if (observedEntries !== diskEntries || observedEntries !== totalEntries) {
    throw new Error(
      `ZIP entry count does not match the central directory (${totalEntries} declared, ${observedEntries} observed).`,
    );
  }
  return observedEntries;
}

export async function loadBoundedHwpxZip(
  bytes: Uint8Array,
  loader: BoundedZipLoader = defaultZipLoader,
): Promise<JSZip> {
  assertClassicZipEntryBudget(bytes);
  return await loader(Uint8Array.from(bytes));
}

function findExactEocd(view: DataView): number {
  if (view.byteLength < EOCD_MINIMUM_BYTES) {
    throw new Error("ZIP end-of-central-directory record is missing.");
  }
  const minimumOffset = Math.max(
    0,
    view.byteLength - EOCD_MINIMUM_BYTES - MAX_ZIP_COMMENT_BYTES,
  );
  for (
    let offset = view.byteLength - EOCD_MINIMUM_BYTES;
...
async function defaultZipLoader(bytes: Uint8Array): Promise<JSZip> {
