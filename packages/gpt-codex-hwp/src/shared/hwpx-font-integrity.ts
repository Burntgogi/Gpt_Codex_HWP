...
export type FontReferenceIssueCode =
  | "FONTFACE_CONTAINER_MISSING"
  | "FONTFACE_COUNT_MISMATCH"
  | "FONTFACE_DUPLICATE"
  | "FONTFACE_MISSING"
  | "FONT_COUNT_MISMATCH"
  | "FONT_ID_INVALID"
  | "FONT_ID_DUPLICATE"
  | "FONT_ID_ZERO_MISSING"
...
export interface FontReferenceInspection {
  issues: FontReferenceIssue[];
}

export interface FontNormalizationChange {
  char_pr_id: string;
  script: HwpScript;
  from: string;
  to: "0";
...
export interface FontNormalizationResult {
  bytes: Uint8Array;
  changed: boolean;
  changed_reference_count: number;
  changes: FontNormalizationChange[];
}

export class HwpxFontReferenceError extends Error {
  readonly code = "HWPX_FONT_REFERENCE_ERROR";

  constructor(
    message: string,
    readonly issues: FontReferenceIssue[],
  ) {
    super(message);
...
export async function inspectHwpxFontReferences(
...
export async function normalizeGeneratedFontReferences(
  ...
  try {
    ...
    const nonRepairable = inspection.issues.filter(
      (issue) => !isRepairableFontReferenceIssue(issue),
    );
    if (nonRepairable.length > 0) {
      throw new HwpxFontReferenceError(
        "Generated HWPX has a font table that cannot be normalized safely.",
        nonRepairable,
      );
    }

    const invalidReferences = inspection.issues.filter(
      (issue): issue is FontReferenceIssue & {
        code: "FONT_REF_INVALID";
        char_pr_id: string;
        script: HwpScript;
        font_id: string;
      } =>
        isRepairableFontReferenceIssue(issue) &&
    ...
    const changes: FontNormalizationChange[] = [];
    for (const issue of invalidReferences) {
      const charPr = charProperties.find(
        (candidate) => candidate.getAttribute("id") === issue.char_pr_id,
      );
      ...
      if (
        ...
        throw new HwpxFontReferenceError(
          "Generated HWPX changed while font references were being normalized.",
          [issue],
        );
      }
...
async function loadArchiveAndHeader(
  input: ArrayBuffer | Uint8Array,
  loadZip?: BoundedZipLoader,
): Promise<{ bytes: Uint8Array; zip: JSZip; document: Document }> {
...
function inspectDocument(document: Document): FontReferenceInspection {
  const issues: FontReferenceIssue[] = [];
  ...
  const idsByLanguage = new Map<string, Set<string>>();

  for (const [, language] of SCRIPT_LANG) {
    const matchingFontfaces = fontfaces.filter(
      (candidate) => candidate.getAttribute("lang") === language,
    );
  ...
  const seenCharPrIds = new Set<string>();
...
function directHeaderChildren(parent: Element, localName: string): Element[] {
...
function isRepairableFontReferenceIssue(
  issue: FontReferenceIssue,
): issue is FontReferenceIssue & {
  code: "FONT_REF_INVALID";
  char_pr_id: string;
  script: HwpScript;
  font_id: string;
